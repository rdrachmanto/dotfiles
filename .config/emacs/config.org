...
** Diminish
Decrease clutter in the mode line by diminishing or hiding the active minor modes. =defer= is applied so this package will be the last one loaded, so the =diminish= command would work properly. 
